...
def normalize_name(name: str) -> str:
    if not name: return ""
    name_str = "".join(c for c in unicodedata.normalize("NFD", name) if unicodedata.category(c) != "Mn")
...
def get_person(name: str, cache: Dict[str, MockPerson]) -> Optional[MockPerson]:
    ...
    if names_in_cache:
        ...
        best_norm_match, score = process.extractOne(normalized_input, normalized_list, scorer=fuzz.token_sort_ratio)
        
        if score >= 90:
            original_name = normalized_to_original[best_norm_match]
            print(f"DEBUG: Fuzzy match for '{name}': '{original_name}' (score: {score})")
