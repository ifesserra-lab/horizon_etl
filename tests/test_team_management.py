...
def test_normalize_name(project_loader):
    ...
    assert (
        project_loader._normalize_name("Pãulo Sérgio Junior") == "PAULO SERGIO JUNIOR"
    )
    assert (
        project_loader._normalize_name("Maria-Aparecida Santos!")
...
def test_get_or_create_person_new_creation(project_loader):
    ...
    assert result.id == 99
    project_loader.person_controller.create_person.assert_called_with(
        name="Novo Usuario"
    )
