...
| **RF-28** | O sistema deve baixar os currículos Lattes dos pesquisadores utilizando a biblioteca `scriptLattes` (v0.7.1). O processo deve gerar os arquivos de configuração e lista de pesquisadores a partir do banco de dados, e salvar o JSON gerado no diretório `data`. | Arquivos JSON gerados no diretório `data`. Configuração e lista geradas dinamicamente. | User Req. |
| **RF-29** | O sistema deve extrair Projetos de Pesquisa, Extensão e Desenvolvimento do currículo Lattes (JSON) e persistí-los como Iniciativas. | Projetos extraídos e classificados corretamente (Research, Extension, Development) e vinculados ao pesquisador. | User Req. |
