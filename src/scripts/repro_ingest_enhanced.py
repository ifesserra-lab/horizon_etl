...
def run_repro():
    ...
    target_file = (
        "data/lattes_json/00_Paulo-Sergio-dos-Santos-Junior_8400407353673370.json"
    )
    ...
    try:
        result = session.execute(
            text(
                "SELECT degree, institution, organization_id, thesis_title FROM academic_educations WHERE researcher_id = 604"
            )
        ).fetchall()
        for row in result:
