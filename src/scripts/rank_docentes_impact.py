...
from src.scripts.analyze_venues import (
    LATTES_DIR, REF_DIR, norm_issn, load_qualis, load_scimago,
)
from src.scripts.generate_docentes_executive import ROSTER_IDS

OUT = Path(__file__).resolve().parents[2] / "data" / "exports" / "docentes" / "ranking_impacto.json"
...
def main() -> None:
    ...
    ap.add_argument("--by-area", action="store_true", help="também mostra o top 1 por área")
    ...
    print(f"  {'#':>2} {'score':>5} {'A1-A4':>5} {'art':>4} {'Q1Q2':>4}  {'docente':<34} {'área'}")
    for r in top:
        det = f"A1:{r['A1']} A2:{r['A2']} A3:{r['A3']} A4:{r['A4']}"
        print(f"  {r['rank']:>2} {r['score_qualis']:>5} {r['estrato_A']:>5} {r['artigos']:>4} "
    ...
    if args.by_area:
        ...
        for area, r in sorted(best.items(), key=lambda x: -x[1]["score_qualis"]):
            print(f"  {area[:34]:<34} {r['nome'][:30]:<30} score={r['score_qualis']} A={r['estrato_A']}")
