...
def _get(url: str) -> dict:
    req = Request(
        url, headers={"User-Agent": f"ifes-serra-research/1.0 (mailto:{MAILTO})"}
    )
...
def collect_dois() -> tuple[dict, dict]:
    ...
    for nome, lid in ROSTER_IDS.items():
        ...
        if not f:
            continue
        for a in (
            json.loads(Path(f).read_text()).get("producao_bibliografica", {}) or {}
...
def fetch_citations(dois: list[str], sleep: float = 0.4) -> dict:
    ...
    for i in range(0, len(uniq), BATCH):
        ...
        flt = "doi:" + "|".join(chunk)
        url = (
            "https://api.openalex.org/works?per-page=200&mailto="
            + MAILTO
        ...
        for w in data.get("results", []):
            ...
            if wdoi:
                ...
                by_year = {
                    c.get("year"): c.get("cited_by_count", 0)
        ...
        print(
            f"  lote {i//BATCH+1}/{(len(uniq)+BATCH-1)//BATCH}: {len(chunk)} DOIs → {len(data.get('results',[]))} achados"
        )
        time.sleep(sleep)
    return out
...
def main() -> None:
    ...
    for lid, nome in ((v, k) for k, v in ROSTER_IDS.items()):
        ...
        found = [
            (d, cit_map[d]) for d in dict.fromkeys(dois) if d in cit_map
        ]  # dedup, achados
        ...
        old_fw = [
            c["fwci"]
            for _, c in found
            if c.get("fwci") is not None and 2016 <= (c.get("year") or 0) <= 2020
        ]
        new_fw = [
            c["fwci"]
            for _, c in found
            if c.get("fwci") is not None and 2021 <= (c.get("year") or 0) <= 2025
        ]
        fwci_antigo = _median(old_fw) if len(old_fw) >= 2 else None
        fwci_recente = _median(new_fw) if len(new_fw) >= 2 else None
        fwci_delta = (
            round(fwci_recente - fwci_antigo, 2)
            if fwci_antigo is not None and fwci_recente is not None
            else None
        )
        ...
        cit_frac = (
            round(
                sum(c["cit"] / max(int(c.get("n_authors") or 1), 1) for _, c in found),
                1,
            )
            if found
            else 0.0
        ...
        rows.append(
            {
                ...
                "fwci_antigo": fwci_antigo,
                "fwci_recente": fwci_recente,
                "fwci_delta": fwci_delta,
    ...
    for r in rows:
        slug = re.sub(
            r"[^a-z0-9]+", "-", r["nome"].lower().encode("ascii", "ignore").decode()
        ).strip("-")
    ...
    for r in rows[:8]:
        print(
            f"  {r['rank']:>2} {r['nome'][:34]:<34} cit={r['citacoes_total']:<5} h={r['h_index']:<3} "
