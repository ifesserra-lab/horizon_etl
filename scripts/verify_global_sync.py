...
def verify():
    ...
    for _, row in df.iterrows():
        ...
        if coord: excel_members.add(normalize_name(coord))
        
        researchers = row['Pesquisadores']
        if researchers:
            for n in str(researchers).split(';'):
                if n.strip(): excel_members.add(normalize_name(n.strip()))
    ...
    if errors:
        ...
        logger.success(f"All {clean_projects}/{total_projects} projects verified as clean!")
